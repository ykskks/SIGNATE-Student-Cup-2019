...
# 各地区ごとのarea, room_floo, parkingrなどのの平均(地区による広さ、高さの定義の違いを考慮する)
# おそらくtrainとtest合わせた平均の方が酔いが、ここは実運用のことを考えてtrainの平均のみを用いた
...
mean_age_in_months_per_district = train_use.groupby("district")["age_in_months"].mean()
train_use["mean_age_in_months_per_district"] = train_use["district"].map(mean_age_in_months_per_district)
test_use["mean_age_in_months_per_district"] = test_use["district"].map(mean_age_in_months_per_district)
...
mean_area_per_district = train_use.groupby("district")["area"].mean()
train_use["mean_area_per_district"] = train_use["district"].map(mean_area_per_district)
test_use["mean_area_per_district"] = test_use["district"].map(mean_area_per_district)
...
mean_building_floor_per_district = train_use.groupby("district")["building_floor"].mean()
train_use["mean_building_floor_per_district"] = train_use["district"].map(mean_building_floor_per_district)
test_use["mean_building_floor_per_district"] = test_use["district"].map(mean_building_floor_per_district)
...
mean_bicycle_parking_per_district = train_use.groupby("district")["bicycle_parking"].mean()
train_use["mean_bicycle_parking_per_district"] = train_use["district"].map(mean_bicycle_parking_per_district)
test_use["mean_bicycle_parking_per_district"] = test_use["district"].map(mean_bicycle_parking_per_district)

mean_car_parking_per_district = train_use.groupby("district")["car_parking"].mean()
train_use["mean_car_parking_per_district"] = train_use["district"].map(mean_car_parking_per_district)
test_use["mean_car_parking_per_district"] = test_use["district"].map(mean_car_parking_per_district)

mean_bike_parking_per_district = train_use.groupby("district")["bike_parking"].mean()
train_use["mean_bike_parking_per_district"] = train_use["district"].map(mean_bike_parking_per_district)
test_use["mean_bike_parking_per_district"] = test_use["district"].map(mean_bike_parking_per_district)

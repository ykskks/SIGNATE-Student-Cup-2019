...
use_cols.extend(["floor_is_underground", "floor_room", "floor_building"])
...
categorical_cols = ['districts', 'direction', 'floor_is_underground', 'シャワー', 'バスなし', 'バス・トイレ別', 
...
for fold, (train_idx, val_idx) in enumerate(folds.split(train)):
    ...
    clf = lgb.train(params, train_data, num_round, valid_sets = [train_data, val_data], verbose_eval=100, early_stopping_rounds=100, callbacks=callbacks)

...
# 区だけでなくもっと詳細な情報(town_average, 過学習を避けるためにtrainに50データ以上ある町以外はまとめる) > leakしてた
# town_cleanedで過学習するのは数が少ないので安定しないから？100データ以上あるやつにする
# 電車路線
# layoutそのまま
# town_cleaned消す
...
# stratified Kfold by town_cleaned
...
### drop outlier ###
train.drop(20926, axis=0, inplace=True) # area>400なのに安すぎる
train.drop([20427, 20231], axis=0, inplace=True) # 500年以上経ってる。。。
...
#train["town"] = train["location"].apply(lambda x: re.search("(?<=都)[一-龥ぁ-んァ-ン]*", x).group())
#test["town"] = test["location"].apply(lambda x: re.search("(?<=都)[一-龥ぁ-んァ-ン]*", x).group())
#over_100_town = train["town"].value_counts().index[train["town"].value_counts() >= 100]
#train["town_cleaned"] = train["town"].apply(lambda x: x if x in over_100_town else "not_in_over_100")
#test["town_cleaned"] = test["town"].apply(lambda x: x if x in over_100_town else "not_in_over_100")
#ce_ordinal = ce.OrdinalEncoder(cols=["town_cleaned"], handle_missing="value")
#train = ce_ordinal.fit_transform(train)
#test = ce_ordinal.transform(test)
#use_cols.append("town_cleaned")
...
train["train_line_list"] = train["access"].apply(lambda x: re.findall("[一-龥ぁ-んァ-ン]*線(?=\t)", x))
test["train_line_list"] = test["access"].apply(lambda x: re.findall("[一-龥ぁ-んァ-ン]*線(?=\t)", x))

train_train_line = pd.get_dummies(train['train_line_list'].apply(pd.Series).stack()).groupby(level=0).apply(sum)
test_train_line = pd.get_dummies(test['train_line_list'].apply(pd.Series).stack()).groupby(level=0).apply(sum)

index_train = train_train_line.sum(axis=0).index[train_train_line.sum(axis=0) >= 100]
train_train_line = train_train_line[index_train]
test_train_line = test_train_line[index_train]

train = pd.concat([train, train_train_line], axis=1)
test = pd.concat([test, test_train_line], axis=1)

use_cols.extend(["mins_to_nearest_sta", "num_sta"])
use_cols.extend(list(train_train_line.columns.values))
...
train = ce_ordinal.fit_transform(train)
test = ce_ordinal.transform(test)
use_cols.extend(["num_room", "has_nando", "is_R", "is_K", "is_DK", "is_LDK", "layout"])

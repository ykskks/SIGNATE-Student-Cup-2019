...
# 区だけでなくもっと詳細な情報(town_average, 過学習を避けるためにtrainに50データ以上ある町以外はまとめる)
...
town_average = dict(pd.concat([train, target], axis=1).groupby("town_cleaned")["rent"].mean())
train["town_average"] = train["town_cleaned"].map(town_average)
test["town_average"] = train["town_cleaned"].map(town_average)
use_cols.append("town_average")

...
for fold, (train_idx, val_idx) in enumerate(folds.split(train_use, train_use["district"])):
	...
	# target encoding by district
    te_train = train_use["district"].iloc[train_idx].to_frame()
    te_train["target"] = target_log[train_idx]
    te_map = te_train.groupby('district')['target'].mean()
    train_use["te_district"] = train_use["district"].map(te_map) # trainの情報だけでtrain,val共にmapする
    test_use["te_district"] = test_use["district"].map(te_map)
